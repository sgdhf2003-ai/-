...
function JingyangWorkflow_tryHandlePostback(event) {
  ...
  if (typeof handleLineReservationPostback === "function") {
    ...
    var res = handleLineReservationPostback({
      ...
      inventoryCatalog: catalog,
      propertiesStorage: props,
      upsertHoldActionFn: typeof JingyangWorkflow_lineCreateHoldBridge_ === "function" ? JingyangWorkflow_lineCreateHoldBridge_ : null
    });
...
function JingyangWorkflow_lineCreateHoldBridge_(holdPayload) {
  if (!holdPayload) {
    return { ok: false, errorCode: "INVALID_BRIDGE_PAYLOAD" };
  }

  var lineUserId = holdPayload.lineUserId;
  var hold = holdPayload.hold;
  ...
  var props = (typeof PropertiesService !== "undefined" && PropertiesService.getScriptProperties)
    ...
    : null;
  var secret = props ? props.getProperty("LINE_BOT_BRIDGE_SECRET") : null;

  if (!secret) {
    return { ok: false, errorCode: "BRIDGE_SECRET_MISSING", message: "後端橋接金鑰尚未設定" };
  }

  var apiUrl = typeof JingyangAssistant_getApiUrl_ === "function"
    ? JingyangAssistant_getApiUrl_()
    : JINGYANG_ASSISTANT_DEFAULT_API_URL;

  var requestBody = {
    action: "lineCreateHold",
    bridgeSecret: secret,
  ...
  try {
    ...
    if (!response || response.getResponseCode() !== 200) {
      return { ok: false, errorCode: "LINE_BRIDGE_REQUEST_FAILED" };
    }
