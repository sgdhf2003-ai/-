/**
 * LINE Reservation Workflow Handler (Apps Script Production Version)
 * 
 * Safety Rules:
  ...
  if (action === "confirmHoldDraft") {
    if (typeof upsertHoldActionFn !== "function") {
      return {
        ...
        success: false,
        errorCode: "BRIDGE_CALLBACK_NOT_CONFIGURED",
        message: "⚠️ 後端劃扣橋接服務尚未設定。"
      };
    }
    ...
    var actionRes = upsertHoldActionFn(holdPayload) || { ok: false };

    if (actionRes && actionRes.ok !== false) {
      propertiesStorage.deleteProperty("pendingDraftHold:" + userId);
      return {
        handled: true,
        action: "confirmHoldDraft",
      ...
      return {
        ...
        action: "confirmHoldDraft",
        success: false,
        errorCode: actionRes.errorCode || "HOLD_CREATION_FAILED",
        message: "⚠️ 劃扣建立失敗：" + (actionRes.message || "未知錯誤")
      };
    }
  }
